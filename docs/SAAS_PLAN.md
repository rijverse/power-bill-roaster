...
Right now Power Roast is just a personal script for me. It polls the DESCO prepaid API every 6 hours and sends a brutally honest email when the electricity balance drops. 
...
The plan:
1. Validate with a free Telegram bot (almost zero cost).
...
- Rely on you remembering to check them
- Crash randomly or have terrible UX
- Don't send reliable proactive alerts
- Are useless if you're a landlord trying to manage 20 different meters
...
This is where the money is. They manage anywhere from 10 to 100+ meters in a building. Right now they have no easy way to see all balances at a glance which leads to disputes with tenants. They have high willingness to pay per meter.
...
- Because consumer ARPU is low our free tier has to cost practically nothing to run.
...
- **The Roast:** Our brutal/funny tone is our marketing hook. B2B users can toggle it to professional mode though.
...
Get this out in weeks for almost no cost. User sends their account + meter number to the bot, sets a threshold, and we're good. No need to build an auth system yet.
...
*Note: We are NOT processing recharges or payments for users (too much regulatory headache right now). No native mobile apps either, responsive web + Telegram is enough.*
...
### Risks
- **DESCO blocks us:** This would kill the project. We have to be extremely polite with their API.
